<template>
    <div class="transition-all duration-300 hover:scale-110 cursor-pointer relative w-full h-40 group">
        <div class="group-hover:absolute top-0 right-0">
            <ActionsData :item="topic" @openModal="emit('openModal', $event)"></ActionsData>
        </div>
        <div :class="`${customColor} w-1/3 h-full rounded-lg absolute bottom-0 left-0`" @click="emit('goTo', {id:topic.id})">
            <component v-if="iconMap[topic.icon]" :is="iconMap[topic.icon]" class="text-white size-6 fill-white relative mx-auto my-[0.4rem]" title="Icon"/>
        </div>
        <div :class="`${customColor} w-full h-[82%] rounded-lg absolute bottom-0 shadow-md`" @click="emit('goTo', topic.id)">
            ...
            <div class="flex flex-row justify-between w-full px-4 py-2 text-sm text-white tracking-wide absolute bottom-0">
                <span v-if="topic.lessons">{{ topic.lessons.length }} lessons</span>
                <span> - </span>
                <span v-if="topic.resources">{{ topic.resources.length }} resources</span>
            </div>
        </div>
    </div>
</template>
<script setup>
    ...
    const customColor = computed(() => {
        return props.topic.color === null ?  'bg-darkPrimary' : props.topic.color
    })
</script>
